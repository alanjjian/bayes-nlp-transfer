...
def run_and_log_bma(net, test_loaders, samples_dir, config, loggers):
    ...
    dict_res = {}
    for test_loader_name in test_loaders.keys():
...
def load_prior(config: dict, prior_ssl_model: SWAG, number_of_samples_prior: int = 4) -> Dict[str, torch.Tensor]:
    "Return the networks prior. Adjusted to work on a singular model."
    prior_type = config['prior_type']
    if prior_type == 'shifted_gaussian':
        
        mean, variance, cov_factor = prior_ssl_model.get_space()
        ...
        model = prior_ssl_model.base_model()
        if 'resnet' in model:
            model = model['resnet']
            del model['fc.weight']
            del model['fc.bias']
