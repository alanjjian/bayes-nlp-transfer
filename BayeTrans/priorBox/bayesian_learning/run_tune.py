...
# from ray.tune.suggest import ConcurrencyLimiter # ray.tune.search in newer versions
# from ray.tune.suggest.hyperopt import HyperOptSearch # ray.tune.search in newer versions
...
def run_ray(args, config, train_loader, val_loader, test_loader):
    ...
    config2 = {'batch_size': tune.choice([32]),
               # 'batch_size': tune.choice([32, 64, 128, 256, 512]), 
               'lr': tune.uniform(0.1, 0.8),
    ...
    reporter = CLIReporter(
        parameter_columns=["batch_size", "momentum", "lr", "weight_decay", "prior_scale"],
    ...
    analysis = tune.run(
        tune.with_parameters(
            training_function, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader),
        resources_per_trial={
